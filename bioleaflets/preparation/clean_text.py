...
def process_section6(package_leaflets_raw):
    ...
    it contains too specific information: contact info and addresses (not interesting to generate)
    ...
    for leaflet in package_leaflets_raw:
        ...
        if section6_content is not None:
            ...
            # if both alternatives of keywords are present
...
def remove_page_numbers(section_content):
    ...
    Note: Important since we treat digits as entities in modelling part.
...
def empty_sections(package_leaflets_unique):
    """
    Count the number of empty sections in bioleaflets dataset

    :param package_leaflets_unique: collection of unique package leaflets
    :return: None
    """
